...
test.describe("Zaphnath Bible Reader UI", async () => {
  test("The main window has navigation sidebar", async ({ page }) => {
    // Wait for the sidebar to be visible
    const sidebar = page.locator('[role="navigation"]').first();
    await expect(sidebar).toBeVisible();
  });

  test("The main window has the application title", async ({ page }) => {
    // Check for the main heading or title
    const title = page.getByText("Zaphnath Bible Reader").first();
    await expect(title).toBeVisible();
  });

  test("Navigation items are present", async ({ page }) => {
    // Check for key navigation items
    const readerLink = page.getByText("Bible Reader");
    const repositoriesLink = page.getByText("Repository Management");
    const settingsLink = page.getByText("Settings");

    await expect(readerLink).toBeVisible();
    await expect(repositoriesLink).toBeVisible();
  ...
  test("Settings page is accessible", async ({ page }) => {
    ...
    const settingsContent = page.getByText("Settings");
