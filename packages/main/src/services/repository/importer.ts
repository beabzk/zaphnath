...
export class RepositoryImporter {
  ...
  public async importRepository(options: ImportOptions): Promise<ImportResult> {
    ...
    try {
      ...
      const manifest = await this.discoveryService.fetchRepositoryManifest(options.repository_url);
      ...
      const existingRepo = this.databaseService.getQueries().getRepository(manifest.repository.id);
      if (existingRepo && !options.overwrite_existing) {
        result.errors.push(`Repository ${manifest.repository.id} already exists. Use overwrite option to replace.`);
        return result;
      }
  ...
  private async validateRepository(manifest: ZBRSManifest, options: ImportOptions): Promise<ValidationResult> {
    // Validate manifest
    ...
    // Validate book count
    if (manifest.content.books_count < 1 || manifest.content.books_count > 100) {
      errors.push({
  ...
  private async importBooks(manifest: ZBRSManifest, options: ImportOptions): Promise<number> {
    let importedCount = 0;
    ...
    for (let bookOrder = 1; bookOrder <= manifest.content.books_count; bookOrder++) {
      try {
        this.reportProgress(options, {
          stage: 'downloading',
        ...
        if (!bookValidation.valid) {
          console.error(`Book ${bookOrder} validation failed:`, bookValidation.errors);
  ...
  private async importBookToDatabase(book: ZBRSBook, repositoryId: string): Promise<void> {
