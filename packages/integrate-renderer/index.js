...
await step('Changing renderer package name to "@app/renderer"', changeRendererPackageName);

await step('Add "--base=./" flag to vite build command', addTheBaseFlagToBuildCommand);

await step('Change the "main" and "exports" property to "./dist/index.html"', addTheMainProperty);
...
function savePkg() {
  fs.writeFileSync('../renderer/package.json', JSON.stringify(pkgJson, null, 2), {
    encoding: 'utf8',
  });
}
